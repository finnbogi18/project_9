...
def open_file(filename):
    """ Tried to open the file, if the file does not exist, prints an error message """
    try:
...
def make_list(file_object):
    ...
    for line in file_object:
        ...
        if len(line_list) == 0: # Appends the paragraph to the main list at the end
            counter += 1
            main_list.append(temp_list)
            temp_list = [] # Resets the temp list to be used again.
...
def paragraph_index(all_words, main_list):
    """ Iterates through the main lists and creates a dictionary for each word and
        in which paragraphs it is. """
    my_dict = {}
...
def word_counter(main_list):
    """ Iterates through the whole list of words and counts how many time each word is used
        in the whole text and returns it as a dictionary. """
    count_dict = {}
    for sublist in main_list:
        for word in sublist:
...
if file_object != None: # the open_file function returns None, the program stops.
    main()